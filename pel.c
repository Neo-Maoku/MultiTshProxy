...
void printHexData(char* data, int size) {
    if (!log_fp) return;  // 如果日志文件未打开，直接返回
    
    fprintf(log_fp, "data length:%x data: %s\n", size, data);
    fprintf(log_fp, "-------------------\n");

    for(int i = 0; i < size; i++) {
        fprintf(log_fp, "%02x ", *(unsigned char*)(data+i));
        if((i + 1) % 16 == 0)
            fprintf(log_fp, "\n");
    }
    fprintf(log_fp, "\n-------------------\n");
    fflush(log_fp);  // 确保立即写入文件
}
